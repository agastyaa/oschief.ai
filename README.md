...
- **AI summaries that are actually useful**  
  After each meeting, OSChief produces a structured write‑up: overview, key points, decisions, action items (with owners and due dates), and open questions. You control the format via prompt templates.
...
- **Hidden from screen share**  
  One toggle to keep OSChief invisible on screen shares so it never becomes the topic of the meeting.
...
- **Knowledge base search**  
  Point OSChief at a folder of notes or reference docs (Obsidian, Notion exports, markdown, PDFs, etc.). During a call, it live‑searches and surfaces relevant talking points so you sound prepared without context‑switching.

- **People & relationships**  
  OSChief automatically extracts the people you meet with. You can merge, edit, and see your history with each person across meetings — a lightweight relationship graph that actually stays up to date.

- **Calendar integration**  
  Connect Google Calendar or Microsoft 365. OSChief pulls upcoming meetings, auto‑detects when one starts, and suggests recording so nothing slips through.
...
- All data stored locally at `~/Library/Application Support/OSChief/`
