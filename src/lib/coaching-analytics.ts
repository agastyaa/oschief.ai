...
/** One evidence-linked bullet from conversation-quality analysis */
export interface ConversationMicroInsight {
  text: string
  framework?: string
...
/** Transcript-grounded moment for jump-to (no audio in v1) */
export interface ConversationKeyMoment {
  title: string
  quote: string
  speaker: string
  time: string
  /** Resolved on the client against note.transcript for scroll-to */
  lineIndex?: number
}

/** LLM + KB output persisted on the note */
...
export interface CoachingMetrics {
  ...
  /** Transcript-grounded Work Coach–style analysis */
